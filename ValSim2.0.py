...
# Listor med uttalanden, de kommer användas senare
def Rösta(): # Funktionen som allt hänger på, simulerar ett val
    for x in Partier:
        x["Röster"] = random.randint(x["Min Röst"], x["Max Röst"]) # För varje dictionary i Partier, skapa ett objekt som heter Röster, och gör den till ett random värde mellan min och max röster för det objektet. Detta bestämmer hur många röster de olika partierna får. 
    def takeRöster(elem): # En funktion som gör att man kan sortera objekten på Röster
        return elem["Röster"]
    Partier.sort(key=takeRöster, reverse = True) # Själva sorteringen
    ...
    for x in Partier: # Alla röster räknas ihop, och alla block får sin egen räkning
        ...
        if x["Block"] == "Oljeblocket":
            ...
            OParti.append(x["partiledare"]) # Lägger till alla partiledare i sorterade listor för att kunna presentera dem senare
...
def takeSecond(elem): # En liknande funktion som den som används för att sortera partierna baserat på röster, men används för att sortera på det andra objektet i listan. Den förra funktionen kan tyvärr inte återanvändas eftersom att denna sorterar i listor, och den andra sorterar i objekt. 
    return elem[1]
...
if len(BlockRöster[1][2]) > 1 :
    ...
    print("ställt, eftersom", BlockRöster[1][0] + " kom på en stark andraplats med", BlockRöster[1][1], "% av rösterna!")
else:
